...
// One-to-one self-FK joins in PostgREST return the joined row as either a
// single object OR a one-element array depending on cardinality detection.
// We always look at the first element (or null) and shape it into our flat
// ListResource form the React component expects.
...
export default async function WindowsResourcesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <AdminShell userEmail={user.email}>
      <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-text-primary">Windows Resources</h1>
          <p className="mt-1 text-sm text-text-muted">Manage Windows download resources.</p>
        </div>
        <Link href="/admin/windows/new">
          <Button>+ Add Windows Resource</Button>
        </Link>
      </header>
          ...
          <div className="flex gap-1 rounded-lg border border-border bg-bg-elevated p-0.5">
            {(['all', 'active', 'inactive'] as const).map((opt) => {
              ...
              return (
                <Link
                  key={opt}
                  href={`/admin/windows?${new URLSearchParams({ ...(q ? { q } : {}), filter: opt }).toString()}`}
                  className={[
      ...
      <div className="mt-4">
        <ResourcesList resources={rows} platform="windows" />
      </div>
    </AdminShell>
  );
}
