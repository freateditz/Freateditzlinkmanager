...
export default async function EditMacResourcePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <AdminShell userEmail={user.email}>
      <header className="mb-6 flex flex-col gap-1">
        ...
        <p className="mt-1 text-sm text-text-muted">
          Update <span className="text-text-primary">{resource.name}</span>.
        </p>
      </header>
      <div className="mb-4 rounded-lg border border-border-subtle bg-bg-elevated/40 px-4 py-3">
        ...
        <p className="mt-1 text-xs text-text-muted">
          Slugs are generated when a resource is created and cannot be edited.
          Platform is locked to Mac.
        </p>
      </div>
      <ResourceForm mode="edit" platform="mac" initial={resource} />
    </AdminShell>
  );
}
